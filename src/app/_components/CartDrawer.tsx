            ...
            <aside className="absolute right-0 top-0 h-full w-full sm:w-[480px] bg-white shadow-2xl flex flex-col">
                    ...
                    {state.items.map((i) => (
                        <div key={i.id} className="flex items-center gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="truncate text-lg font-bold">{i.title}</div>
                                <div className="text-base text-gray-700 font-semibold">${i.price.toFixed(2)} USD</div>
                            </div>
                ...
                <div className="border-t px-4 py-4 space-y-3">
                    ...
                    <div className="flex gap-3">
                        <Link href={`/${lang}/cart`} className="flex-1 rounded-full border px-5 py-3.5 text-center text-lg font-bold transition-colors hover:bg-indigo-600 hover:text-white hover:border-indigo-600" onClick={onClose}>View Cart</Link>
                        <Link href={`/${lang}/checkout`} className="flex-1 rounded-full bg-black px-5 py-3.5 text-center text-lg font-bold text-white transition-colors hover:bg-indigo-700" onClick={onClose}>Checkout</Link>
                    </div>
                </div>
            </aside>
        </div>
