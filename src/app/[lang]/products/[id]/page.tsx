                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-4">
                    <div className="lg:col-span-7 space-y-4">
                        ...
                        <div className="pt-4 flex flex-wrap items-center justify-center gap-3 text-sm">
                            <div className="w-auto inline-flex items-center gap-2 rounded-full bg-blue-50 text-blue-700 px-3 py-1.5 hover:bg-blue-600 hover:text-white transition-colors">
                                ...
                                <span>Ships directly from Tokyo</span>
                            </div>
                            <div className="w-auto inline-flex items-center gap-2 rounded-full bg-blue-50 text-blue-700 px-3 py-1.5 hover:bg-blue-600 hover:text-white transition-colors">
                                <span className="h-2 w-2 rounded-full bg-current"></span>
                    ...
                    <div className="lg:col-span-5">
                        ...
                        <div className="text-lg mb-2 flex items-center gap-2">
                            <span className={`${product.compareAt ? "text-rose-600 font-semibold" : "text-black font-semibold"}`}>${product.price.toFixed(2)} USD</span>
                            {product.compareAt && (
                                <span className="text-base text-black line-through">${product.compareAt.toFixed(2)} USD</span>
                            )}
                        </div>
                    ...
                    <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between gap-4">
                        <div className="min-w-0">
                            ...
                            <div className="text-sm flex items-center gap-2">
                                <span className={`${product.compareAt ? "text-rose-600 font-semibold" : "text-black font-semibold"}`}>${product.price.toFixed(2)} USD</span>
                                {product.compareAt && (
                                    <span className="text-xs text-black line-through">${product.compareAt.toFixed(2)} USD</span>
                                )}
                            </div>
                        </div>
